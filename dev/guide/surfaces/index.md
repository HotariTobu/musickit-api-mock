...
Accepted shapes vary by field — some accept only a response value or callable; others also accept a `Mapping` keyed by id (a plain `dict`). See each field's type in the [reference](https://hotaritobu.github.io/musickit-api-mock/dev/reference/endpoints/index.md) for the exact union.

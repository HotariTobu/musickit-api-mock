...
1. **Inject the in-page JS shim** via `add_init_script`, which replaces MusicKit JS's browser integrations (EME, Media Source, authorize popup) so the page can run end-to-end without real Apple Music infrastructure.

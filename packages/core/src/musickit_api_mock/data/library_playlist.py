...
class LibraryPlaylist:
    ...
    Attributes:
        ...
        track_ids: Library song ids in the playlist's track order. ``None``
            and an empty list both describe an empty playlist.
        artwork: Cover artwork.
        description: Playlist description.
        catalog_id: Catalog playlist id linking back to the catalog
            counterpart, used to resolve ``?include=catalog`` and emitted
            as ``playParams.globalId``.
    """
...
class _LibraryPlaylistResolver:
    ...
    def list_ids(self) -> list[str]:
        """Return every library playlist id; requires a mapping source."""
        return _list_source_ids(self._get_source(), "data.library_playlists")

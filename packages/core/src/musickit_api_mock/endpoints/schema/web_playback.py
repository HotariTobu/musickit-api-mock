...
def _web_playback_asset(a: WebPlaybackAsset) -> WebPlaybackAssetEntry:
    out: WebPlaybackAssetEntry = {"flavor": a.flavor, "URL": a.url}
    if a.metadata is not None:
        out["metadata"] = a.metadata
    if a.artwork_url is not None:
        out["artworkURL"] = a.artwork_url
    return out
...
def _web_playback_song(s: WebPlaybackSong) -> WebPlaybackSongEntry:
    if isinstance(s, WebPlaybackUploadedLibrarySong):
        assets = [_web_playback_uploaded_asset(s.asset)]
        if s.artwork_url is None:
            return {"songId": -1, "needsPlaybackReporting": False, "assets": assets}
        return {
            "songId": -1,
    ...
    if s.hls_playlist_url is not None:
        out["hls-playlist-url"] = s.hls_playlist_url
    if isinstance(s, WebPlaybackCatalogLibrarySong):
        out["needsPlaybackReporting"] = True
    return out


def _web_playback_success_body(resp: WebPlaybackResponseSuccess) -> WebPlaybackBody:
    return {"songList": [_web_playback_song(s) for s in resp.song_list], "status": 0}


def _web_playback_unsupported_body() -> WebPlaybackBody:
    """Empty-songList success shape for the unsupported-error variant."""
    return {"songList": [], "status": 0}


def _web_playback_failure_body(code: int) -> WebPlaybackBody:

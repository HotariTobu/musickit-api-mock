...
async def test_eme_flavor_endpoint_serves_live_value(
    mount_async_page: Callable[..., Awaitable[AsyncPage]], page_url: str
) -> None:
    mock = MusicKitApiMock()
    ...
    await page.goto(page_url)
    assert_eme_flavor_success(
        await page.evaluate(FETCH_EME_FLAVOR), "com.widevine.alpha"
    )
...
async def test_static_fairplay_flavor_installs_legacy_detection_synchronously(
    mount_async_page: Callable[..., Awaitable[AsyncPage]], page_url: str
) -> None:
    ...
    page = await mount_async_page(mock)
    await page.add_init_script(EARLY_SHIM_PROBE_INIT_SCRIPT)
    await page.goto(page_url)
    assert_legacy_fairplay_sync_installed(await page.evaluate(READ_EARLY_SHIM_STATE))


async def test_static_playready_flavor_installs_legacy_detection_synchronously(
    mount_async_page: Callable[..., Awaitable[AsyncPage]], page_url: str
) -> None:
    ...
    await page.add_init_script(EARLY_SHIM_PROBE_INIT_SCRIPT)
    await page.goto(page_url)
    assert_legacy_playready_sync_installed(await page.evaluate(READ_EARLY_SHIM_STATE))

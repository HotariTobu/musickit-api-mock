"""Boundary-surface intercept tests: drive the mock with raw ``fetch`` / EME calls.

These exercise the mock's HTTP / shim boundary independently of MusicKit JS,
giving unit-grade coverage that survives MusicKit version drift. End-to-end
behavior through MusicKit JS lives in ``test_musickit_sync.py``.
"""
...
pytestmark = pytest.mark.sync_test


def test_storefront_request_intercepted(
    ...
    page.goto(page_url)
    assert_storefront_response(page.evaluate(FETCH_STOREFRONT))


def test_unintercepted_apple_unrelated_url_falls_through(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    caught: list[str] = []

    def catcher(route: Route) -> None:
        caught.append(route.request.url)
        route.fulfill(status=200)

    page = mount_sync_page(
        mock,
        lambda p: p.route(UNRELATED_APPLE_URL, catcher),
    )
    page.goto(page_url)
    page.evaluate(FETCH_UNRELATED_APPLE_URL)
    assert caught == [UNRELATED_APPLE_URL]


def test_license_failure_injection(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_license_failure(page.evaluate(FETCH_LICENSE), -1017)


def test_license_success_returns_base64(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
...
def test_authorize_response_endpoint_serves_live_value(
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_authorize_response_success(page.evaluate(FETCH_AUTHORIZE_RESPONSE), "ut")


def test_shim_overrides_request_media_key_system_access(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    mock.browser.eme_flavor = "com.widevine.alpha"
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_widevine_granted(page.evaluate(REQUEST_WIDEVINE_ACCESS))


def test_shim_rejects_non_active_flavor(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    mock.browser.eme_flavor = "com.widevine.alpha"
...
def test_oauth_popup_shim_dispatches_authorize_message(
    ...
    page.goto(page_url)
    assert_authorize_message(
        page.evaluate(OPEN_OAUTH_POPUP_AND_RECEIVE), "my-token", "my-cid"
    )


def test_handler_error_aborts_fetch(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_fetch_aborted(page.evaluate(FETCH_STOREFRONT_AND_REPORT_OUTCOME))
...
def test_genre_singular_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_genre_singular_response(page.evaluate(FETCH_GENRE_SINGULAR))


def test_data_dict_mutation_after_intercept_is_reflected(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    ...
    result = page.evaluate(FETCH_GENRE_SINGULAR)
    assert result["status"] == 200
    assert json.loads(result["body"])["data"][0]["attributes"]["name"] == "Rock"


def test_data_callable_mutation_after_intercept_is_reflected(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
...
def test_endpoint_static_mutation_after_intercept_is_reflected(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    ...
    result = page.evaluate(FETCH_STOREFRONT)
    assert result["status"] == 200
...
def test_endpoint_keyed_mutation_after_intercept_is_reflected(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    ...
    assert_license_success_b64(page.evaluate(FETCH_LICENSE), b"world")


def test_recommendation_singular_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page.goto(page_url)
    assert_recommendation_singular_response(
        page.evaluate(FETCH_RECOMMENDATION_SINGULAR)
    )


def test_catalog_song_library_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_song_library_response(page.evaluate(FETCH_SONG_LIBRARY))
...
def test_artist_station_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_artist_station_response(page.evaluate(FETCH_ARTIST_STATION))


def test_music_video_songs_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_music_video_songs_response(page.evaluate(FETCH_MUSIC_VIDEO_SONGS))


def test_album_record_labels_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_album_record_labels_response(page.evaluate(FETCH_ALBUM_RECORD_LABELS))


def test_apple_curator_playlists_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page.goto(page_url)
    assert_apple_curator_playlists_response(
        page.evaluate(FETCH_APPLE_CURATOR_PLAYLISTS)
    )


def test_library_artist_albums_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_library_artist_albums_response(page.evaluate(FETCH_LIBRARY_ARTIST_ALBUMS))


def test_station_radio_show_endpoint_intercepted(
    mount_sync_page: Callable[..., Page], page_url: str
) -> None:
    mock = MusicKitApiMock()
    _basic_storefront(mock)
    ...
    page = mount_sync_page(mock)
    page.goto(page_url)
    assert_station_radio_show_response(page.evaluate(FETCH_STATION_RADIO_SHOW))
